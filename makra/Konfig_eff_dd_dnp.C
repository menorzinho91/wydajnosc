...
int Konfig_eff_all() {
	...
	int col=15;
	float wiersz[col];
//	int numery_plikow1[]={17,19,21,23,25,27,29};
//	int numery_plikow2[]={17,19,21,23,25,27,29};
	int numery_plikow1[]={17,19};
	int numery_plikow2[]={17,19};
int num1;
int num2;
TFile *wyniki_all;
wyniki_all= new TFile("./Wyniki_all/Wyniki_calosc.root","RECREATE");
TTree *calosc_wynikow=new TTree("cal_wyn","cale wyniki");
int Theta_1;
int Theta_2;
int Dphi;
float effi;
float deffi;
int IsECutOn;
calosc_wynikow->Branch("TH1",&Theta_1,"Theta_1/s");
calosc_wynikow->Branch("TH2",&Theta_2,"Theta_2/s");
calosc_wynikow->Branch("Dphi",&Dphi,"Dphi/s");
calosc_wynikow->Branch("Eff",&effi,"effi/F");
calosc_wynikow->Branch("dEff",&deffi,"deffi/F");
calosc_wynikow->Branch("IsECutOn",&IsECutOn,"IsECutOn/s");


fstream wyniki;
TCanvas *c2 = new TCanvas("c2","srednie wydajnosci",200,10,700,600);
c2->Divide(2,2);
   wyniki.open("./Wyniki_all/Wyniki_calosc.txt", ios::app | ios::out );
   if( wyniki.good() == true )
   {
	for(int iter=0;iter<1;iter++){
	cout<<"iteracja numer= "<<iter<<"\n";
	for(num1=0;num1<sizeof(numery_plikow1)/sizeof(int);num1++){
	cout<<"num1= "<<num1<<"\n";
	for(num2=num1;num2<sizeof(numery_plikow2)/sizeof(int);num2++){
	cout<<"num2= "<<num2<<"\n";
	...
	sprintf(nazwa2,"./Wyniki_all/Symulacja_%d_%d_%d.root",iter,numery_plikow1[num1],numery_plikow2[num2]);
...
for(int z=0;z<2;z++){
	...
	Double_t S_g;
fstream plik;
sprintf(nazwa,"./Wyniki_all/Wynik_%d_%d_%d.txt",iter,numery_plikow1[num1],numery_plikow2[num2]);
...
for(int i=0;i<idfimax;i++){
	...
	Theta_1=numery_plikow1[num1];
	Theta_2=numery_plikow2[num2];
	Dphi=(i+1)*10;
	effi=fi_wyd_srednie[i];
	deffi=fi_niep_srednie[i];
	IsECutOn=iter;
	calosc_wynikow->Fill();
}
wyniki_all->Write();
...
sprintf(title,"#theta_{1}=%d #theta_{2}=%d",numery_plikow1[num1],numery_plikow2[num2]);
		   gr->SetMarkerColor(4);
			if(iter==1)gr->SetMarkerColor(2);
			if(iter==2)gr->SetMarkerColor(6);
