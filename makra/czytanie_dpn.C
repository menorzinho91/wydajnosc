...
for(konf=0;konf<sizeof(th1)/sizeof(*th1);konf++){
	TFile *F;
	sprintf(file1,"../wyniki/Raw_Sim_dpn_%d_%d.root",th1[konf],th2[konf]);
	...
	TTree *Raw_Tree=new TTree("Raw","Surowe_wyniki");
		...
		//sprintf(file2,"../%d_%d/Bina_pd_out1.dat",th1[konf],th2[konf]);
		...
		if(th1[konf]!=th2[konf]){
			...
			sprintf(file2,"../%d_%d/Bina_pd_out1.dat",th2[konf],th1[konf]);
			cout<<file2<<"\n";
			stream1.open(file2);
			if(stream1.good()){
			for (int i = 0; i < col; i++) 
...
//Analiza
	...
	*/
	...
	TH1F *E2_proj= new TH1F("E2_proj","Projekcja energii protonu",200,1,201);
	TH1F *E3_proj= new TH1F("E3_proj","Projekcja energii deuteronu",200,1,201);
